...
def create_custom_training_job_from_component(
  ...
  Args:
    component_spec: The KFP Component object to run as Vertex AI custom job.
    display_name (str): The name of the custom job. If not provided the
      component_spec.name will be used instead.
    replica_count (int): The count of instances in the cluster. One replica
      always counts towards the master in worker_pool_spec[0] and the remaining
      replicas will be allocated in worker_pool_spec[1]. For more details see
      https://cloud.google.com/vertex-ai/docs/training/distributed-training#configure_a_distributed_training_job.
    machine_type (str): The type of the machine to run the custom job. The
      default value is "n1-standard-4".  For more details about this input
      config, see
      https://cloud.google.com/vertex-ai/docs/training/configure-compute#machine-types.
    accelerator_type (str): The type of accelerator(s) that may be attached to
      the machine as per accelerator_count.  For more details about this input
      config, see
    ...
    boot_disk_type (str): Type of the boot disk (default is "pd-ssd"). Valid
      ...
      (Persistent Disk Hard Disk Drive). boot_disk_type is set as a static value
      and cannot be changed as a pipeline parameter.
    boot_disk_size_gb (int): Size in GB of the boot disk (default is 100GB).
      ...
      pipeline parameter.
    timeout (str): The maximum job running time. The default is 7 days. A
    ...
    service_account (str): Sets the default service account for workload run-as
    ...
    network (str): The full name of the Compute Engine network to which the job
    ...
    encryption_spec_key_name (str): Customer-managed encryption key options for
      the CustomJob. If this is set, then all resources created by the CustomJob
      will be encrypted with the provided encryption key.
    tensorboard (str): The name of a Vertex AI Tensorboard resource to which
      this CustomJob will upload Tensorboard logs.
    enable_web_access (bool): Whether you want Vertex AI to enable [interactive
      shell
    ...
    base_output_directory (str): The Cloud Storage location to store the output
      of this CustomJob or HyperparameterTuningJob. see below for more details:
