...
def GetBestTrialOp(trials: list, study_spec_metrics: list) -> str:
  ...
  Args:
      trials (list): Required. List representing the intermediate JSON
        representation of the trials from the hyperparameter tuning job.
      study_spec_metrics (list): Required. List serialized from dictionary
        representing the metrics to optimize. The dictionary key is the
        metric_id, which is reported by your training job, and the dictionary
        value is the optimization goal of the metric ('minimize' or 'maximize').
        example: metrics = hyperparameter_tuning_job.serialize_metrics( {'loss':
        'minimize', 'accuracy': 'maximize'})
...
def GetWorkerPoolSpecsOp(best_hyperparameters: list,
  ...
  Args:
      best_hyperparameters (list): Required. List representing the intermediate
        JSON representation of the best hyperparameters from the hyperparameter
        tuning job.
      worker_pool_specs (list): Required. The spec of the worker pools including
        machine type and Docker image. All worker pools except the first one are
...
def serialize_parameters(parameters: dict) -> list:
  ...
  Args:
      parameters (Dict[str, hyperparameter_tuning._ParameterSpec]): Dictionary
        ...
        import hyperparameter_tuning as hpt parameters={ 'decay':
        hpt.DoubleParameterSpec(min=1e-7, max=1, scale='linear'),
        'learning_rate': hpt.DoubleParameterSpec(min=1e-7, max=1,
        scale='linear') 'batch_size': hpt.DiscreteParamterSpec(values=[4, 8, 16,
        32, 64, 128], scale='linear') } Supported parameter specifications can
