...
def bigquery_ml_principal_component_info_job(
  ...
  """Launch a BigQuery ML.principal_component_info job and waits for it to finish.
    ...
    Args:
        project (str): Required. Project to run BigQuery
          ML.principal_component_info job.
        location (Optional[str]): Location to run the BigQuery
  ...
  return ContainerSpec(
      ...
      command=[
          'python3', '-u', '-m',
          'google_cloud_pipeline_components.container.v1.bigquery.ml_principal_component_info.launcher'
      ],
      args=[
          '--type',
