...
def use_config_map_as_env(
    ...
    <https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/#define-container-environment-variables-using-configmap-data>`__.
    ...
    config_map_as_env = pb.ConfigMapAsEnv(
        key_to_env=key_to_env, optional=optional)

    config_map_name_parameter = common.parse_k8s_parameter_input(
        config_map_name, task)
    config_map_as_env.config_map_name_parameter.CopyFrom(
        config_map_name_parameter)
...
def use_config_map_as_volume(
    ...
    described by the `Kubernetes documentation <https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/#add-configmap-data-to-a-volume>`__.
    ...
    config_map_name_parameter = common.parse_k8s_parameter_input(
        config_map_name, task)
    config_map_as_vol.config_map_name_parameter.CopyFrom(
        config_map_name_parameter)
