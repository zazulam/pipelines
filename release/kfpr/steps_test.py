...
class StepSelectionTest(unittest.TestCase):
    ...
    def test_patch_includes_cherry_pick_steps(self):
        steps_list = [
            step.step_id for step in steps.build_steps(
                'patch', include_backend=True, include_sdk=True)
        ]
    ...
    def test_minor_keeps_release_order_and_updates_docs(self):
        steps_list = [
            step.step_id for step in steps.build_steps(
                'minor', include_backend=True, include_sdk=True)
        ]
        self.assertLess(
            steps_list.index('publish-sdks'),
            steps_list.index('create-backend-release'))
        self.assertLess(
            steps_list.index('create-sdk-tag'),
            steps_list.index('publish-sdks'))
        self.assertLess(
    ...
    def test_major_skips_cherry_pick_steps(self):
        steps_list = [
            step.step_id for step in steps.build_steps(
                'major', include_backend=True, include_sdk=True)
        ]
    ...
    def test_backend_only_patch_skips_sdk_steps(self):
        steps_list = [
            step.step_id for step in steps.build_steps(
                'patch', include_backend=True, include_sdk=False)
        ]
        self.assertIn('publish-images', steps_list)
    ...
    def test_step_registry_contains_every_built_step(self):
        for release_type in ('major', 'minor', 'patch'):
            for step in steps.build_steps(
                    release_type, include_backend=True, include_sdk=True):
    ...
    def test_cherry_pick_rejects_pr_without_merge_commit(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(
                    RuntimeError, 'Could not find merge commit for PR 123'):
...
class PublishImagesStepTest(unittest.TestCase):
    ...
    def test_publish_images_prints_workflow_run_url(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with mock.patch(
                    'time.time', return_value=1783537500), mock.patch(
...
class CreateSdkReleaseStepTest(unittest.TestCase):
    ...
    def test_create_sdk_release_reuses_existing_release_when_user_accepts(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn([
                'gh', 'release', 'view', 'sdk-3.2.0', '--repo',
                'kubeflow/pipelines'
            ], context.runner.commands)
            self.assertNotIn(
                ['gh', 'release', 'create', 'sdk-3.2.0'],
                [command[:4] for command in context.runner.commands])
    ...
    def test_create_sdk_release_uses_existing_tag_when_user_accepts(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn([
                'gh', 'api', '--silent',
                'repos/kubeflow/pipelines/git/ref/tags/sdk-3.2.0'
            ], context.runner.commands)
            self.assertNotIn(['git', 'push', '--delete', 'origin', 'sdk-3.2.0'],
                             context.runner.commands)
            self.assertNotIn(
                ['gh', 'release', 'create', 'sdk-3.2.0'],
                [command[:4] for command in context.runner.commands])
    ...
    def test_create_sdk_release_recreates_existing_tag_when_requested(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(['git', 'fetch', 'upstream', 'release-3.2'],
                          context.runner.commands)
            self.assertIn(
                ['git', 'tag', '-f', 'sdk-3.2.0', 'upstream/release-3.2'],
                context.runner.commands)
            self.assertIn([
                'git', 'push', '--force',
                'https://github.com/kubeflow/pipelines.git', 'sdk-3.2.0'
            ], context.runner.commands)
            self.assertNotIn(
                ['gh', 'release', 'create', 'sdk-3.2.0'],
                [command[:4] for command in context.runner.commands])

    def test_create_sdk_release_pushes_missing_tag_before_creating_release(
            self):
        with TemporaryDirectory() as tmpdir:
...
class PublishSdksStepTest(unittest.TestCase):
    ...
    def test_publish_sdks_reruns_once_when_pypi_packages_are_missing(self):
        with TemporaryDirectory() as tmpdir:
            ...
            class NoopRunner(core.CommandRunner):

                def run(self, command, cwd=None, check=True):
                    self.commands.append(command)
                    return subprocess.CompletedProcess(command, 0, '', '')

            runner = NoopRunner(dry_run=False)
            context = core.ReleaseContext(
                root=Path(tmpdir),
                state=core.ReleaseState(Path(tmpdir) / 'state.json'),
                runner=runner,
            ...
            workflow_commands = [
                command for command in runner.commands if command[:4] ==
    ...
    def test_pypi_package_check_uses_package_version_json(self):
        opened_urls = []
        ...
        with mock.patch('urllib.request.urlopen', side_effect=fake_urlopen):
            self.assertTrue(steps._is_pypi_version_published('kfp', '3.2.0'))

        self.assertEqual(opened_urls, ['https://pypi.org/pypi/kfp/3.2.0/json'])


class SyncMasterStepTest(unittest.TestCase):

    def test_sync_master_opens_pr_and_waits_for_merge(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)

            class SyncMasterRunner(core.CommandRunner):

                def capture(self, command, cwd=None):
                    self.commands.append(command)
                    if command[:3] == ['gh', 'pr', 'create']:
                        return 'https://github.com/kubeflow/pipelines/pull/123'
                    raise AssertionError(f'unexpected capture: {command}')

            context = core.ReleaseContext(
                root=root,
                state=core.ReleaseState(root / 'state.json'),
                runner=SyncMasterRunner(dry_run=True),
                metadata=core.ReleaseMetadata.from_version('minor', '3.2.0'),
                fork_remote='git@github.com:testuser/pipelines.git',
                include_backend=True,
                include_sdk=True,
            ...
            with mock.patch.object(
                    steps, 'watch_pr_ci') as watch_ci, mock.patch.object(
                        steps, 'wait_for_pr_merge') as wait_merge:
                steps.step_sync_master(context)

            self.assertIn([
                'git', 'push', '--set-upstream',
                'git@github.com:testuser/pipelines.git',
                'chore-release-3.2.0-master'
            ], context.runner.commands)
            self.assertIn(['git', 'checkout', '3.2.0', '--', 'CHANGELOG.md'],
                          context.runner.commands)
            self.assertIn(['gh', 'pr', 'create'],
                          [command[:3] for command in context.runner.commands])
            watch_ci.assert_called_once_with(
                context.runner,
                'https://github.com/kubeflow/pipelines/pull/123')
            wait_merge.assert_called_once_with(
                context.runner,
                'https://github.com/kubeflow/pipelines/pull/123')

    def test_sync_master_prompts_for_local_review_before_commit(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            context = core.ReleaseContext(
                root=root,
                state=core.ReleaseState(root / 'state.json'),
                runner=SyncMasterRunner(dry_run=False),
                metadata=core.ReleaseMetadata.from_version('minor', '3.2.0'),
                fork_remote='git@github.com:testuser/pipelines.git',
                include_backend=True,
                include_sdk=True,
            ...
            with mock.patch(
                    'sys.stdin.isatty', return_value=True), mock.patch(
                        'builtins.input',
                        return_value='y') as input_mock, mock.patch.object(
                            steps, 'watch_pr_ci'), mock.patch.object(
                                steps, 'wait_for_pr_merge'):
                steps.step_sync_master(context)

            status_index = context.runner.commands.index(
                ['git', 'status', '--short'])
            commit_index = context.runner.commands.index([
                'git', 'commit', '-s', '-m',
                'chore(release): bump version to 3.2.0 on master branch'
            ])
            self.assertLess(status_index, commit_index)
            input_mock.assert_called()

    def test_sync_master_skip_local_review_bypasses_commit_prompt(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            state = core.ReleaseState(root / 'state.json')
            state.answers['skip_local_review'] = True
            ...
            with mock.patch('builtins.input') as input_mock, mock.patch.object(
                    steps,
                    'watch_pr_ci'), mock.patch.object(steps,
                                                      'wait_for_pr_merge'):
                steps.step_sync_master(context)

            self.assertNotIn(['git', 'status', '--short'],
                             context.runner.commands)
...
class PrepareReleaseBranchStepTest(unittest.TestCase):
    ...
    def test_prepare_release_branch_uses_selected_source_branch(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(['git', 'checkout', 'release-candidate'],
                          context.runner.commands)
            self.assertNotIn(['git', 'checkout', 'master'],
                             context.runner.commands)
    ...
    def test_prepare_release_branch_skips_pull_without_source_upstream(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn(['git', 'pull', '--ff-only'],
                             context.runner.commands)
            self.assertIn(['git', 'checkout', '-b', 'release-3.2'],
                          context.runner.commands)
...
class OrchestrationTest(unittest.TestCase):
            ...
            for include_backend in [True, False]:
                for include_sdk in [True, False]:
                    with self.subTest(
                            ...
                            sdk=include_sdk):
                        steps_list = steps.build_steps(release_type,
                                                       include_backend,
                                                       include_sdk)
                        ...
                        if include_backend:
                            ...
                            self.assertNotIn('confirm-website-and-slack',
                                             step_ids)
    ...
    def test_run_steps_dry_run_adds_blank_line_between_steps(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with mock.patch.object(
                    steps, 'build_steps', return_value=fake_steps):
                with mock.patch.object(
                        steps,
                        'noop_step',
                        create=True,
                        side_effect=lambda context: None):
...
class StepPRCreationTest(unittest.TestCase):
    ...
    def test_step_merge_version_pr_dry_run_uses_fork_head(self):
        with TemporaryDirectory() as tmpdir:
            ...
            class RecordingRunner:
                ...
                def capture(self, command, cwd=None):
                    self.commands.append(command)
                    if 'view' in command and any(
                            'statusCheckRollup' in part for part in command):
                        return '[{"name":"ci","status":"COMPLETED","conclusion":"SUCCESS"}]'
                    if 'view' in command and any(
                            'mergeStateStatus' in part for part in command):
                        return 'MERGED CLEAN'
    ...
    def test_step_merge_version_pr_pushes_missing_upstream_release_branch(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(
                [
                    'git', 'push', 'https://github.com/kubeflow/pipelines.git',
                    'release-1.3'
                ],
                runner.commands,
            )

    def test_step_merge_version_pr_skips_existing_upstream_release_branch_push(
            self):
        with TemporaryDirectory() as tmpdir:
    ...
    def test_step_merge_version_pr_uses_existing_remote_branch_when_requested(
            self):
        with TemporaryDirectory() as tmpdir:
    ...
    def test_step_merge_version_pr_recreates_existing_remote_branch_when_requested(
            self):
        with TemporaryDirectory() as tmpdir:
    ...
    def test_step_merge_version_pr_uses_existing_remote_tag_when_requested(
            self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn([
                'git', 'push', 'git@github.com:testuser/pipelines.git', '1.3.0'
            ], runner.commands)
            self.assertNotIn([
                'git', 'push', '--delete',
                'git@github.com:testuser/pipelines.git', '1.3.0'
            ], runner.commands)

    def test_step_merge_version_pr_recreates_existing_remote_tag_when_requested(
            self):
        with TemporaryDirectory() as tmpdir:
...
class StepSdkVersionFilesTest(unittest.TestCase):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            def fake_check_output(command, cwd=None, text=True):
                ...
                if command[:2] == ['git', 'log']:
                    ...
                    return ('feat(sdk): add local execution support (#123)\n'
    ...
    def test_generate_backend_release_notes_from_non_sdk_conventional_commits(
            self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            def fake_check_output(command, cwd=None, text=True):
                ...
                if command[:2] == ['git', 'log']:
                    ...
                    return ('feat(backend): add recurring run support (#123)\n'
            ...
            with mock.patch(
                    'subprocess.check_output', side_effect=fake_check_output):
                notes = steps._generate_backend_release_notes(root, '1.2.3')
    ...
    def test_update_sdk_version_files_replaces_version_without_trailing_garbage(
            self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            self.assertEqual(version_file.read_text(),
                             "__version__ = '1.2.4'\n")
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            files = {
                ...
                'backend/api/v2beta1/python_http_client/kfp_server_api/__init__.py':
                    '__version__ = "1.2.3"\n',
                'sdk/python/requirements.in': ('kfp-pipeline-spec>=1.2.3,<3\n'
                ...
                'kubernetes_platform/python/docs/conf.py': (
                    ...
                    "    ],\n"
                    "}\n"),
                'sdk/RELEASE.md': ('# Current Version (in development)\n\n'
                                   '## Features\n\n'
                                   '* New SDK feature.\n\n'
                                   '## Breaking changes\n\n'
                                   '## Deprecations\n\n'
            ...
            class RecordingRunner:
                ...
                def run(self, command, cwd=None, check=True):
                    ...
                    if command[:4] == [
                            ...
                            'refs/heads/1.3.0-update-version-tags'
                    ]:
                        return subprocess.CompletedProcess(command, 1, '', '')
                    if command[:4] == [
                            'git', 'rev-parse', '--verify', 'refs/tags/1.3.0'
                    ]:
                        return subprocess.CompletedProcess(command, 1, '', '')
                    return subprocess.CompletedProcess(command, 0, '', '')
            ...
            self.assertEqual((root / 'sdk/python/kfp/version.py').read_text(),
                             "__version__ = '1.3.0'\n")
            self.assertEqual(
                (root / 'kubernetes_platform/python/kfp/kubernetes/__init__.py'
                ).read_text(), "__version__ = '1.3.0'\n")
            self.assertIn("VERSION = '1.3.0'",
                          (root / 'api/v2alpha1/python/setup.py').read_text())
            self.assertIn(
                'VERSION = "1.3.0"',
                (root /
                 'backend/api/v2beta1/python_http_client/setup.py').read_text())
            self.assertIn('__version__ = "1.3.0"', (
                root /
                'backend/api/v2beta1/python_http_client/kfp_server_api/__init__.py'
            ).read_text())
            self.assertIn('kfp-pipeline-spec>=1.3.0,<2',
                          (root / 'sdk/python/requirements.in').read_text())
            self.assertIn('kfp-server-api>=1.3.0,<2',
                          (root / 'sdk/python/requirements.in').read_text())
            self.assertEqual(
                (root /
                 'kubernetes_platform/python/requirements.in').read_text(),
                'kfp>=1.3.0,<2\n')
            self.assertIn('"title": "1.3.0"',
                          (root / 'docs/sdk/versions.json').read_text())
            self.assertIn('"aliases": []',
                          (root / 'docs/sdk/versions.json').read_text())
            self.assertIn(
                "'title':\n                '1.3'",
                (root / 'kubernetes_platform/python/docs/conf.py').read_text())
            self.assertIn(
                "'aliases': []",
                (root / 'kubernetes_platform/python/docs/conf.py').read_text())
            self.assertIn('# 1.3.0', (root / 'sdk/RELEASE.md').read_text())
            self.assertIn('* New SDK feature.',
                          (root / 'sdk/RELEASE.md').read_text())
        ...
        for dry_run in (False, True):
            with self.subTest(dry_run=dry_run), TemporaryDirectory() as tmpdir:
                ...
                for package, path in (
                    ...
                    runner.run.assert_any_call([
                        'uv', 'build', '--package', package, '--out-dir',
                        f'{path}/dist'
                    ],
                                               cwd=root)
...
class DryRunOutputTest(unittest.TestCase):
    ...
    def test_update_version_tags_cuts_update_branch_from_release_branch(self):
        ...
        class MissingBranchRunner(core.CommandRunner):
            ...
            def run(self, command, cwd=None, check=True):
                ...
                if command[:4] == [
                        'git', 'rev-parse', '--verify',
                        'refs/heads/3.2.0-update-version-tags'
                ]:
                    return subprocess.CompletedProcess(command, 1, '', '')
                if command[:4] == [
                        'git', 'rev-parse', '--verify', 'refs/tags/3.2.0'
                ]:
                    return subprocess.CompletedProcess(command, 1, '', '')
                return subprocess.CompletedProcess(command, 0, '', '')
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(context.runner.commands[0],
                             ['git', 'checkout', 'release-3.2'])
            self.assertIn(['git', 'pull', '--ff-only'], context.runner.commands)
            self.assertIn(
                ['git', 'checkout', '-b', '3.2.0-update-version-tags'],
                context.runner.commands)

    def test_update_version_tags_skips_pull_without_release_branch_upstream(
            self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(['git', 'checkout', 'release-3.2'],
                          context.runner.commands)
            self.assertNotIn(['git', 'pull', '--ff-only'],
                             context.runner.commands)
            self.assertIn(
    ...
    def test_update_version_tags_recreates_existing_update_branch_when_requested(
            self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(['git', 'branch', '-D', '3.2.0-update-version-tags'],
                          context.runner.commands)
            self.assertIn(
                ['git', 'checkout', '-b', '3.2.0-update-version-tags'],
                context.runner.commands)

    def test_update_version_tags_uses_existing_update_branch_when_requested(
            self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(['git', 'checkout', '3.2.0-update-version-tags'],
                          context.runner.commands)
            self.assertNotIn(
                ['git', 'branch', '-D', '3.2.0-update-version-tags'],
                context.runner.commands)
            self.assertNotIn(
                ['git', 'checkout', '-b', '3.2.0-update-version-tags'],
                context.runner.commands)
    ...
    def test_update_version_tags_no_longer_edits_release_makefile(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(
                any('test/release/Makefile' in command
    ...
    def test_update_version_tags_pins_release_dockerfile_image(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                any('release/Dockerfile.release' in command and
                    'release-3.2' in command for command in flattened_commands),
                flattened_commands,
            )
            self.assertFalse(
                any('test/release/Dockerfile.release' in command
                    for command in flattened_commands),
                flattened_commands,
            )
    ...
    def test_update_version_tags_pins_tool_images_to_release_branch(self):
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                any('PREBUILT_REMOTE_IMAGE=ghcr.io/kubeflow/kfp-api-generator:'
                    in command and 'release-3.2' in command
                    for command in flattened_commands),
                flattened_commands,
            )
            self.assertTrue(
                any('RELEASE_IMAGE=ghcr.io/kubeflow/kfp-release:' in command and
                    'release-3.2' in command for command in flattened_commands),
                flattened_commands,
            )
            self.assertFalse(
    ...
    def test_update_version_tags_recreates_existing_tag_when_requested(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(['git', 'tag', '-d', '3.2.0'],
                          context.runner.commands)
            self.assertIn(
                [
                    'git', 'tag', '-a', '3.2.0', '-m',
                    'Kubeflow Pipelines 3.2.0 release'
                ],
                context.runner.commands,
            )
    ...
    def test_update_version_tags_uses_existing_tag_when_requested(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn(['git', 'tag', '-d', '3.2.0'],
                             context.runner.commands)
            self.assertNotIn(
                [
                    'git', 'tag', '-a', '3.2.0', '-m',
    ...
    def test_manual_checkpoint_text_can_be_reused_by_status(self):
        ...
        self.assertIn('ReadTheDocs manual checkpoint:',
                      steps.manual_checklist('confirm-rtd', metadata))
        self.assertIn(
            'https://app.readthedocs.org/projects/kubeflow-pipelines/',
            steps.manual_checklist('confirm-rtd', metadata))
        final_checklist = steps.manual_checklist('confirm-website-and-slack',
                                                 metadata)
        self.assertIn('Manual final checkpoint:', final_checklist)
        self.assertIn(
            'https://github.com/kubeflow/website/edit/master/layouts/shortcodes/pipelines/latest-version.html',
            final_checklist)
        self.assertIn('KFP 3.2.0 has officially been released!',
                      final_checklist)
        self.assertIn('Backend - ', final_checklist)
        self.assertIn(
            'https://github.com/kubeflow/pipelines/releases/tag/3.2.0',
            final_checklist)
        self.assertIn('SDK - ', final_checklist)
        self.assertIn(
            'https://github.com/kubeflow/pipelines/releases/tag/sdk-3.2.0',
            final_checklist)
        self.assertIn('Changes: ', final_checklist)
        self.assertIn(
            'https://github.com/kubeflow/pipelines/blob/master/CHANGELOG.md',
            final_checklist)
    ...
    def test_final_confirmation_prints_release_completion_message(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with contextlib.redirect_stdout(
                    io.StringIO()) as output, mock.patch(
                        'builtins.input', return_value='y'):
                steps.step_confirm_website_and_slack(context)

            self.assertIn("Congratulations! You've completed the release!",
                          output.getvalue())
    ...
    def test_confirm_rtd_automates_with_prompted_token(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with mock.patch(
                    'getpass.getpass',
                    return_value='secret-token'), mock.patch.object(
                        steps.rtd,
                        'ReadTheDocsClient') as client_cls, mock.patch.object(
                            steps.rtd,
                            'update_release_docs') as update_docs, mock.patch(
                                'builtins.print') as print_mock:
    ...
    def test_confirm_rtd_uses_env_token_without_prompting(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with mock.patch.dict(
                    'os.environ',
                {'RTD_TOKEN': 'env-token'}), mock.patch(
                    'getpass.getpass') as getpass_mock, mock.patch.object(
                        steps.rtd,
                        'ReadTheDocsClient') as client_cls, mock.patch.object(
                            steps.rtd, 'update_release_docs') as update_docs:
                steps.step_confirm_rtd(context)
    ...
    def test_confirm_rtd_reasks_for_empty_token(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with mock.patch(
                    'getpass.getpass',
                    side_effect=['', 'secret-token']), mock.patch.object(
                        steps.rtd,
                        'ReadTheDocsClient') as client_cls, mock.patch.object(
                            steps.rtd, 'update_release_docs'):
                steps.step_confirm_rtd(context)

            client_cls.assert_called_once_with('secret-token')

    def test_confirm_rtd_falls_back_to_manual_when_automation_fails_and_user_accepts(
            self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            context = core.ReleaseContext(
            ...
            with mock.patch(
                    'getpass.getpass',
                    return_value='secret-token'), mock.patch.object(
                        steps.rtd, 'ReadTheDocsClient'), mock.patch.object(
                            steps.rtd,
                            'update_release_docs',
                            side_effect=steps.rtd.ReadTheDocsError(
                                'boom')), mock.patch(
    ...
    def test_kfp_kubernetes_docs_branch_reuses_existing_remote_branch(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(context.runner.commands, [
                [
                    'git', 'ls-remote', '--exit-code', '--heads', 'upstream',
                    'kfp-kubernetes-3.2'
                ],
            ])

    def test_kfp_kubernetes_docs_branch_can_replace_existing_remote_branch(
            self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            (root / 'kubernetes_platform/python/docs/.readthedocs.yml'
            ...
            self.assertIn(['git', 'fetch', 'upstream', 'release-3.2'],
                          context.runner.commands)
            self.assertIn([
                'git', 'checkout', '-B', 'release-3.2', 'upstream/release-3.2'
            ], context.runner.commands)
    ...
    def test_kfp_kubernetes_docs_branch_can_force_push_existing_remote_branch(
            self):
    ...
    def test_merge_version_pr_dry_run_does_not_print_empty_pr_wait(self):
        with TemporaryDirectory() as tmpdir:
            ...
            output = '\n'.join(
                str(call.args[0]) for call in print_mock.call_args_list)
            self.assertIn('[dry-run] would open PR: version bump for 3.2.0',
                          output)
