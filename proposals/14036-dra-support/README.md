...
- `resource_claim_template_name`: required, name of a `ResourceClaimTemplate` in the same namespace. A new `ResourceClaim` is created per pod and deleted with the pod.
- Can be called multiple times on the same task to attach multiple claims.
- The pod-local claim name (`PodResourceClaim.Name`) defaults to `resource_claim_template_name`.
...
- `resource_claim_template_name`: required, name of a `ResourceClaimTemplate` in the same namespace.
- Raises `ValueError` if `resource_claim_template_name` is empty.
...
spec:
  resourceClaims:
    - name: gpu-claim-template         # defaults to resource_claim_template_name
...
message PodResourceClaim {
  ...
  ml_pipelines.TaskInputsSpec.InputParameterSpec resource_claim_json = 2;
}
...
3. Convert to `corev1.PodResourceClaim` structs, setting `Name` to `ResourceClaimTemplateName` for each claim.
4. Validate all resolved claims: `ResourceClaimTemplateName` must be non-empty. Return an error if validation fails.
...
- Validation: error when `resource_claim_template_name` is empty

**`ResourceClaimConfig`:**
- Valid `resource_claim_template_name` creates config successfully
- Empty `resource_claim_template_name` raises `ValueError`
- Missing `resource_claim_template_name` raises error
...
- Static variant: field mapping from proto (`resource_claim_template_name`) to K8s struct (`ResourceClaimTemplateName`)
- Claim `Name` defaults to `ResourceClaimTemplateName`
...
- Empty `resource_claim_template_name` — verify backend returns error
- JSON variant with empty or missing `resourceClaimTemplateName` — verify backend returns error
...
- Mixed static + JSON claims on same task — verify merged list
